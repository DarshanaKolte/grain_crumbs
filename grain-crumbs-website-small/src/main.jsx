...
function App() {
  ...
  const order = () => {
    const text = cartItems.length
      ? `Hi Grain Crumbs, I want to order:%0A${cartItems
          .map((item) => `${item.qty} x ${item.name} - ₹${item.price * item.qty}`)
          .join("%0A")}%0ATotal: ₹${total}`
      : "Hi Grain Crumbs, I want to place an order.";
  ...
  return (
    <>
      <nav>
        <img src={logo} alt="Grain Crumbs Logo" />
        ...
        <button onClick={() => setOpen(true)}>
          <ShoppingBag /> Cart {cartItems.length}
        </button>
      ...
      <section className="hero">
        <div className="heroText">
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="eyebrow">
            Millet-based • Jaggery sweetened • Freshly baked
          </motion.p>

          <motion.h1 initial={{ opacity: 0, y: 25 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
            Handcrafted brownies, sponge cakes & cupcakes baked with real ingredients.
          </motion.h1>

          <p>
            Grain Crumbs brings beautiful bakery treats with millet flour, no maida, no refined sugar and no
            preservatives — perfect for mindful gifting, celebrations and everyday cravings.
          </p>
        ...
        <div className="mixWrap">
          <motion.div className="bowl" initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }}>
            <div className="cream"></div>
            <motion.div
              className="whisk"
              animate={{ rotate: [-25, 25, -25], x: [-8, 8, -8] }}
      ...
      <section className="badges" id="why">
        {[
          ["Millet Flour Only", Wheat],
          ["Jaggery Sweetened", Heart],
          ["Low GI Spike", ShieldCheck],
          ["Vegan & Veg Options", Leaf],
          ["No Maida", Wheat],
          ["No Refined Sugar", ShieldCheck],
          ["No Preservatives", Leaf],
        ].map(([text, Icon]) => (
          <motion.div whileHover={{ y: -8 }} className="badge" key={text}>
            <Icon />
            <span>{text}</span>
          </motion.div>
        ))}
      ...
      <section className="menuSec" id="menu">
        <p className="eyebrow">Millet-Based Brownies</p>
        <h2>Brownie Menu</h2>

        <div className="grid">
          {products.map((product) => (
            <motion.div className="card" whileHover={{ y: -8 }} key={product.id}>
              <div className="prodImg" style={{ backgroundImage: `url(${product.img})` }}></div>
              <h3>{product.name}</h3>
              <p>{product.desc}</p>
      ...
      <section className="split" id="gallery">
        <img src={orderPoster} alt="Grain Crumbs order poster" />

        <div>
          <p className="eyebrow">Signature Brand Promise</p>
          <h2>Made with real ingredients. Baked with love.</h2>
          <p>
            Use this website to show the menu, build trust, highlight healthy bakery features, collect orders and
            redirect customers directly to WhatsApp for final confirmation.
          </p>
          <button onClick={order}>Start Order</button>
        </div>
      </section>
      ...
      <section className="contact" id="contact">
        ...
        <div className="contactGrid">
          ...
          <a href="https://instagram.com/graincrumbs" target="_blank" rel="noreferrer">
      ...
      {open && (
        <div className="drawer">
          <div className="cart">
            ...
            <h3>Total: ₹{total}</h3>
            <button className="checkout" onClick={order}>
